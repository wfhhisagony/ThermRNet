...
import sys
import logging
from detect_module_respirationrate import rr_detect_config
...
import socket
import threading
from detect_module_respirationrate.rr_detector import MyYoloPoseModel
...
DETECT_FRAME_NUM = 160  # 约6s的滑动窗口
STRIDE = DETECT_FRAME_NUM // 2  # 使得后续检测约3s进行一次
...
class RRDetectorRealtime:
    def __init__(self, thermal_frame_width=THERMAL_FRAME_WIDTH, thermal_frame_height=THERMAL_FRAME_HEIGHT,
                 thermal_fps=THERMAL_FPS, detect_frame_num=DETECT_FRAME_NUM, stride=STRIDE):
        self.rect = (  (0, 0), (1, 1)  )
        ...
        self.physiology_detector = PhysiologyDetector()
        self.physiology_detector.init(['rr'])
...
if __name__ == '__main__':
    ...
    rr_detector_realtime = RRDetectorRealtime()
